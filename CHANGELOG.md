...
- **README's "Capture itself never blocks your coroutines" is now true.**
  ...
  loop ~26 ms on this machine, scaling with body size. After: at 256 KB
  and 1 MB bodies (HTTP/1 and HTTP/2, 32 concurrent streams) the
  wardex-attributed loop-drift p99 stays within the 0.5 ms target on
  this machine (0.0-0.35 ms across runs); at the harshest measured shape
  — 32 simultaneous 4 MB streams, 128 MB transiting the loop per round —
  a residual attributed p99 of roughly 1-5 ms remains (h2 ~1 ms, h1 up
  to ~4.6 ms measured), because the bytes still cross the loop even
  though the parse does not. Not size-independent at that fan-in, and
  not claimed to be.
