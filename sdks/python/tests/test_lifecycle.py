"""Runtime — atexit single registration, signal chaining, re-init teardown."""
...
def real_signal_table():
    ...
    Separate from `clean_lifecycle`, which resets the RUNTIME, and depended on
    by it so that this one is set up first and torn down LAST — the poisoning
    below happens *inside* `clean_lifecycle`'s teardown, so a fixture that
    finalized before it would restore the table and then watch it be re-broken.

    Several tests here drive `_runtime._handler` by writing a disposition into
    the runtime's `_prev_handlers[SIGTERM]` by hand, which is the only way to
    ...
    `_runtime._handler` is frequently sitting in the table when this file
...
def clean_lifecycle(real_signal_table):
    """Empty the runtime around every test.

    One call, where this used to be four lines that reached into module globals
    by hand — clear the lifecycle's client slot, close whatever it held, drop
    the signal handlers, then reset the hub's separate client slot. Those two
    slots are one slot now, and `reset_for_test()` goes through the object that
    owns it, so this fixture no longer has to know which pieces exist.
    """
    _hub.reset_for_test()
    yield
    _hub.reset_for_test()
...
def test_install_closes_previous_client_and_flushes_it():
    ...
    assert _runtime.current_client() is second


def test_atexit_registered_exactly_once(monkeypatch):
    calls = []
    monkeypatch.setattr(_runtime.atexit, "register", lambda fn: calls.append(fn))
    monkeypatch.setattr(_runtime.runtime(), "_atexit_registered", False)
    c1 = _client()
    _runtime.runtime().install(c1, c1.config)
    c2 = _client()
    _runtime.runtime().install(c2, c2.config)
    assert len(calls) == 1
...
def test_signal_install_skipped_off_main_thread():
    ...
    assert not _runtime.runtime()._signals_installed
    assert signal.getsignal(signal.SIGINT) is not _runtime._handler
...
def test_partial_signal_install_rolls_back(monkeypatch):
    ...
    def failing_signal(signum, handler):
        if signum == signal.SIGTERM and handler is _runtime._handler:
            raise OSError("no SIGTERM here")
        return real_signal(signum, handler)

    monkeypatch.setattr(_runtime.signal, "signal", failing_signal)
    before = signal.getsignal(signal.SIGINT)
    c = _client()
    _runtime.runtime().install(c, c.config)
    assert not _runtime.runtime()._signals_installed
    assert _runtime.runtime()._prev_handlers == {}
...
def test_signal_handler_chains_even_if_flush_raises(monkeypatch):
    ...
    try:
        ...
        _runtime._handler(signal.SIGINT, None)  # must not raise, must still chain
...
def test_reinit_uninstalls_interceptors_before_closing_previous_client():
    ...
    Re-init (`Runtime.install` with a live previous client) must uninstall
...
def test_reinit_uninstalls_adapters_before_closing_previous_client():
    """Regression: re-init (`Runtime.install` with a live previous client)
...
def test_teardown_closes_units_while_the_client_can_still_send_them(unit_adapter):
    ...
    _runtime.runtime()._teardown(client)
...
def test_the_signal_handler_closes_live_units_when_the_process_is_about_to_die(unit_adapter):
    ...
    _runtime.runtime()._prev_handlers[signal.SIGTERM] = signal.SIG_IGN  # do not kill pytest
    _runtime._handler(signal.SIGTERM, None)
...
def test_the_signal_handler_leaves_units_open_when_the_app_carries_on(unit_adapter):
    ...
    seen: list = []
    _runtime.runtime()._prev_handlers[signal.SIGTERM] = lambda signum, frame: seen.append(signum)
    _runtime._handler(signal.SIGTERM, None)
...
def test_an_adapter_whose_close_units_raises_cannot_take_the_flush_with_it(unit_adapter):
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr(os, "kill", lambda *a: None)
        _runtime._handler(signal.SIGTERM, None)  # must not raise
