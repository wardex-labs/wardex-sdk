...
third: the byte seams' close hook registers one per connection, so SDK code can
now re-enter at an arbitrary ALLOCATION and on ANY thread, not only on the main
one. Neither of those two reaches this lock today. The self-recursion does, and
it is what makes the reentrancy a property of the lock rather than a standing
claim about who calls it.
