...
belongs beside it — in `_assembly/_emit.py`, so that one place decides whether a
span ships instead of each caller deciding for itself; `tests/test_import_graph.py`
reserves that name and asserts no module under `_assembly/` reaches the sink
...
from .._assembly import SpanDraft
