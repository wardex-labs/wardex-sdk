...
`_assembly/` imports the leaf vocabulary
(`_types`, `_enums`, `_limits`) and the scope layer (`_hub`, `_scope`,
`context/`) and nothing above it — never `_interceptors/`, `_adapters/`,
`_protocol/` or `_semantics/` (design §3.1). `tests/test_import_graph.py`
