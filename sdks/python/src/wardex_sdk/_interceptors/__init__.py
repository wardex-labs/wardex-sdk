...
# This package is private — the runtime's composition root is its only caller,
# and `install_configured_interceptors` is the one entry point it uses. The
# interface, the registry and the seam classes are reached by their defining
# modules (`._base`, `._registry`, `._ssl`, ...) by the internal code that
# needs them; nothing here re-exports them, so the package's surface cannot be
# mistaken for a supported one. The old eager `SSLInterceptor` re-export (and
# the PEP 562 hook that later kept it lazily) went with the privatization: the
# lazy resolution existed to keep a PUBLIC spelling alive without dragging the
# TLS seam — and the native extension underneath it — into every import of
# this package, and a private package has no public spelling to keep.
__all__ = [
